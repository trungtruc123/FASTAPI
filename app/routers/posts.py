...
def get_post(db: Session = Depends(get_db), current_user: int = Depends(oauth2.get_current_user),
    ...
    my_posts = db.query(models.Post, func.count(models.Vote.post_id).label("number_votes")).join(models.Vote,
...
def get_one_post(id: int, db: Session = Depends(get_db), current_user: int = Depends(oauth2.get_current_user)):
    ...
    post = db.query(models.Post, func.count(models.Vote.post_id).label("number_votes")).join(models.Vote,
                                                                                                 models.Post.id == models.Vote.post_id,
                                                                                                 isouter=True).group_by(
